...
def UserQuestion(request):
    try:
        ...
        print(subquestion)
        return render(request, "UserQuestions.html", {'question':question[0],'subquestion':subquestion,'length':len(subquestion),'qnum':0,'scorelist':[],'qlength':qlength,'splid':splid,'splname':question[0]['specialization']})
    except Exception as e:
        print("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
        print(e)
        return render(request, "UserQuestions.html", {'question':[], 'subquestion': []})
